...
export function FilePickerModal({
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      formData.append(
        "isPublic",
        category.includes("photo") ? "true" : "false"
      );
      ...
      if (data.success) {
        ...
        if (data.optimization) {
          ...
          toast.success(
            `File uploaded successfully! Optimized: ${saved}% smaller (saved ${formatFileSize(
              data.optimization.savedBytes
            )})`,
            { duration: 5000 }
          );
        } else {
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList
              className={`grid w-full ${
                [allowUpload, allowBrowse, allowUrl, allowRecent].filter(
                  Boolean
                ).length === 4
                  ? "grid-cols-4"
                  : [allowUpload, allowBrowse, allowUrl, allowRecent].filter(
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    {filteredServerFiles.length > 0 && (
                      ...
                      >
                        {selectedFileIds.size === filteredServerFiles.length
                          ? "Deselect All"
                          : "Select All"}
                      </Button>
                    )}
