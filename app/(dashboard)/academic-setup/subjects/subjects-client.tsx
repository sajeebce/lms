...
import { createSubject, updateSubject, deleteSubject } from '@/lib/actions/subject.actions'
    ...
    <div className="space-y-4">
        ...
        <Table>
          <TableHeader>
            <TableRow className="border-b border-border dark:border-slate-700">
              <TableHead className="text-foreground dark:text-slate-300">Icon</TableHead>
              <TableHead className="text-foreground dark:text-slate-300">Name</TableHead>
              <TableHead className="text-foreground dark:text-slate-300">Code</TableHead>
              <TableHead className="text-foreground dark:text-slate-300">Status</TableHead>
              <TableHead className="text-foreground dark:text-slate-300">Courses</TableHead>
              <TableHead className="text-foreground dark:text-slate-300">Chapters</TableHead>
              <TableHead className="text-foreground dark:text-slate-300 text-right">Actions</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {filteredSubjects.length === 0 ? (
              ...
              filteredSubjects.map((subject) => (
                <TableRow key={subject.id} className="border-b border-border dark:border-slate-700 hover:bg-muted/50 dark:hover:bg-slate-700/30">
                  <TableCell>
                  ...
                  <TableCell>
                    ...
                    >
                      {subject.status === 'ACTIVE' ? '✅ Active' : '⏸️ Inactive'}
                    </Badge>
                  </TableCell>
      ...
      <Dialog open={addDialogOpen} onOpenChange={setAddDialogOpen}>
        <DialogContent className="dark:bg-slate-900 max-w-2xl">
          <DialogHeader>
            <DialogTitle className="dark:text-slate-100">Add Subject</DialogTitle>
          </DialogHeader>
      ...
      <Dialog open={editDialogOpen} onOpenChange={setEditDialogOpen}>
        <DialogContent className="dark:bg-slate-900 max-w-2xl">
          <DialogHeader>
            <DialogTitle className="dark:text-slate-100">Edit Subject</DialogTitle>
          </DialogHeader>
          {editingSubject && (
            <SubjectForm
        ...
        <AlertDialogContent className="dark:bg-slate-900">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="dark:text-slate-400">
              Are you sure you want to delete &quot;{deletingSubject?.name}&quot;? This action
              cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
