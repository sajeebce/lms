...
## 🎯 PHASE 1: IMAGE ENHANCEMENTS (MATCH SUN EDITOR)

**Goal:** Make image editing equal to or better than Sun Editor

### **1.0 Fix Theme Color Integration** (~20 lines) - **PREREQUISITE**
**Current:** Math buttons (LaTeX, MathLive) and Image button use fixed colors
**Target:** Use theme CSS variables for consistency

**Implementation:**
- 🔲 Update LaTeX button: Remove `text-purple-600` → Use theme variables
- 🔲 Update MathLive button: Remove `from-violet-600 to-orange-500` → Use `from-[var(--theme-button-from)] to-[var(--theme-button-to)]`
- 🔲 Update Image button: Remove `text-blue-600` → Use theme variables or `variant="default"`

**Files to modify:**
- `app/(dashboard)/question-bank/questions/_components/math-editor.tsx` (lines 898, 911, 924)

**Code Impact:** ~20 lines (just className changes)
**Performance Impact:** Zero
**Priority:** 🔴 High (consistency issue)

---

### **1.1 Enhanced Resize Handles** (~100 lines)
**Current:** 4 corner handles only
**Target:** 8 handles (4 corners + 4 sides)

**Implementation:**
- ✅ **Corner handles (NW, NE, SW, SE)** - Maintain aspect ratio (already done)
- 🔲 **Side handles (N, E, S, W)** - Free resize (ratio can change)
- 🔲 **Visual distinction** - Different cursor styles (nwse-resize vs ns-resize)
- 🔲 **Live preview** - Show dimensions while dragging

**Files to modify:**
- `app/(dashboard)/question-bank/questions/_components/math-editor.tsx` (ResizableImage extension)
...
### **1.2 Image Description Field** (~50 lines)
**Current:** Only alt text (accessibility)  
**Target:** Separate description field (like Sun Editor)

**Implementation:**
- 🔲 Add `description` field to ImagePropertiesDialog
- 🔲 Store in `data-description` attribute
- 🔲 Show description below image (optional toggle)
- 🔲 Use for image gallery/library metadata
...
### **1.3 Size Display** (~20 lines)
**Current:** No dimension display  
**Target:** Show "Basic (457px, 437px)" or "Basic (auto, auto)"

**Implementation:**
- 🔲 Add dimension display in ImagePropertiesDialog
- 🔲 Show current size in floating toolbar (optional)
- 🔲 Update live when resizing
...
### **1.4 Border/Frame Options** (~30 lines)
**Current:** No border options  
**Target:** Border width, color, style (solid/dashed/dotted)

**Implementation:**
- 🔲 Add border controls to ImagePropertiesDialog
- 🔲 Store in `data-border` attribute
- 🔲 Apply CSS border styles
- 🔲 Preset styles (None, Thin, Medium, Thick)
...
**Phase 1 Total:**
- **Lines of code:** ~220 lines (including theme fix)
- **Performance impact:** Zero (no Canvas API, just DOM/CSS)
- **Bundle size increase:** 0 KB
- **Time estimate:** 2-3 hours

**⚠️ Important:** Phase 1.0 (theme color fix) should be done FIRST before other features to ensure consistency.
...
### **2.1 Rotate Left/Right** (~150 lines)
**Current:** No rotation  
...
zoomBtn.addEventListener('click', () => {
  ...
  dropdown.className = 'absolute bg-white dark:bg-slate-800 border rounded shadow-lg p-2 z-50'
  ...
  zoomLevels.forEach((level) => {
    ...
    option.className = 'block w-full text-left px-3 py-1 hover:bg-slate-100 dark:hover:bg-slate-700 rounded'
