...
export default function MathLiveModal({
  open,
  onClose,
  onInsert,
}: MathLiveModalProps) {
  ...
  useEffect(() => {
    ...
    loadMathLiveModule()
      ...
      .catch((error) => {
        ...
        if (isActive) {
          setLoadError(
            "Failed to load the visual Equation editor. Please retry."
          );
        }
      });
    ...
    >
      ...
      >
        <DialogHeader>
          ...
          <DialogDescription>
            Use the visual editor below or click templates to insert math
            equations
          </DialogDescription>
        </DialogHeader>
